...
def generate_dict_list(args):
    ...
    for line in sessions_file: 
        ...
        if len(entry_array) >= 6 and entry_array[2] == 'Q':
            ...
            for doc in docs:
                if not (doc in doc_uid):
                    doc_uid[doc] = len(doc_uid)
            interaction_info['docs'] = docs
            interaction_info['uids'] = [doc_uid[doc] for doc in docs]
            # process clicks
        ...
        # If the entry has 4 elements it is a click
            ...
            if int(entry_array[0]) == info_per_session['session_id']:
                clicked_uid = doc_uid[int(entry_array[3])]
    ...
    print('  - {}'.format('save and check query_qid, doc_uid...'))
    print('    - {}'.format('unique query number: {}'.format(len(query_qid))))
    print('    - {}'.format('unique doc number: {}'.format(len(doc_uid))))
    save_dict('data/', 'query_qid.dict', query_qid)
    save_dict('data/', 'doc_uid.dict', doc_uid)
    dict1 = load_dict('data', 'query_qid.dict')
    dict2 = load_dict('data', 'doc_uid.dict')
    assert len(query_qid) == len(dict1)
    assert len(doc_uid) == len(dict2)
    for key in query_qid:
        assert dict1[key] == query_qid[key]
        assert type(key) == type(1)
    for key in doc_uid:
        assert dict2[key] == doc_uid[key]
        assert type(key) == type(1)
    
    print('  - {}'.format('Done'))

def generate_data_txt(args):
    # load query_qid & doc_uid
    print('  - {}'.format('loading query_qid & doc_uid...'))
    query_qid = load_dict('data', 'query_qid.dict')
    doc_uid = load_dict('data', 'doc_uid.dict')
    ...
    print('    - {}'.format('writing into data/train_per_session.txt'))
    generate_data_per_session(infos_per_session, indices[:train_dev_split], 'data/', 'train_per_session.txt')
    print('    - {}'.format('writing into data/dev_per_session.txt'))
    generate_data_per_session(infos_per_session, indices[train_dev_split:dev_test_split], 'data/', 'dev_per_session.txt')
    print('    - {}'.format('writing into data/test_per_session.txt'))
    generate_data_per_session(infos_per_session, indices[dev_test_split:], 'data/', 'test_per_session.txt')
    ...
    # np.random.shuffle(indices)
    print('    - {}'.format('writing into data/train_per_query.txt'))
    generate_data_per_query(infos_per_query, indices[0:train_query_num], 'data/', 'train_per_query.txt')
    print('    - {}'.format('writing into data/dev_per_query.txt'))
    generate_data_per_query(infos_per_query, indices[train_query_num:dev_query_num], 'data/', 'dev_per_query.txt')
    print('    - {}'.format('writing into data/test_per_query.txt'))
    generate_data_per_query(infos_per_query, indices[dev_query_num:test_query_num], 'data/', 'test_per_query.txt')
...
def generate_embedding(args):
    ...
    torch.save(query_embedding_QDQD, 'data/query_embedding.emb')
    ...
    torch.save(doc_embedding_QDQD, 'data/doc_embedding.emb')

def main():
    parser = argparse.ArgumentParser('YandexRelPredChallenge')
    parser.add_argument('--dataset', default='../dataset/YandexRelPredChallenge',
                        help='dataset path')
